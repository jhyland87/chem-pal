...
All suppliers extend `SupplierBase`, which defines the lifecycle: `setup()` → `queryProductsResolved()` → `queryProducts()` → `fuzzyFilterAst()` → `initProductBuilders()` → `getProductData()` → `yield product`. Platform-specific base classes (`SupplierBaseWix`, `SupplierBaseSearchanise`, `SupplierBaseShopify`, `SupplierBaseWoocommerce`, `SupplierBaseMagento2`, `SupplierBaseAmazon`) handle common patterns for those e-commerce platforms.
