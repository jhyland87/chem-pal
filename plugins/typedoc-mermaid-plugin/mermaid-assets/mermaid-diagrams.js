...
function valueToPx(value) {
  if (isNumeric(value) || typeof value === "number") return `${value}px`;

  if (typeof value === "string") return value;

  console.warn(`[Mermaid] Invalid px value: ${value}`);
  return "600px";
}

const ZOOM_CONTROLS = ["pan", "wheel", "none"];

/**
 * Normalize a zoomControl value to one of "pan", "wheel", or "none".
 * @param {unknown} value - The raw config value (data attribute or frontmatter).
 * @returns {"pan"|"wheel"|"none"} The validated mode; defaults to "pan".
 */
function normalizeZoomControl(value) {
  const v = String(value == null ? "" : value)
    .trim()
    .toLowerCase();
  if (ZOOM_CONTROLS.includes(v)) return v;
  if (v !== "") console.warn(`[Mermaid] Invalid zoomControl "${value}"; using "pan"`);
  return "pan";
}
...
(function () {
  ...
  // Frontmatter parser, provided by mermaid-frontmatter.js (loaded first). Falls
  // back to a no-op if that script is missing, so diagrams still render on the
  // global config.
  const { parseFrontmatter } = window.MermaidFrontmatter || {
    parseFrontmatter: () => ({ data: {}, content: "" }),
  };

  const isTrue = (value) => value === "true" || value === true;
  const cfg = {
    // String, not Number: CDN version tags like "10.9.1" or "latest" aren't
    // valid numbers. Matches the server's `data-version` attribute.
    version: script.dataset.version || "11",
    zoomControl: normalizeZoomControl(script.dataset.zoomControl),
    minZoom: Number(script.dataset.minZoom || 0.3),
    maxZoom: Number(script.dataset.maxZoom || 10),
    containerHeight: valueToPx(script.dataset.containerHeight || 600),
    disableMaximize: isTrue(script.dataset.disableMaximize),
  };

  // Non-maximized diagram height is a build option; hand it to the stylesheet.
  // This is the page-wide default; a diagram may override it per-block below.
  document.documentElement.style.setProperty("--mermaid-container-height", cfg.containerHeight);

  // ── Per-diagram config ───────────────────────────────────────────────
  // A diagram can override plugin options in its own frontmatter, namespaced
  // under config.mermaidPlugin:
  //   ---
  //   config:
  //     mermaidPlugin:
  //       disableMaximize: true
  //   ---
  // Those keys are merged over the global cfg for that one diagram, then
  // stripped from the source so mermaid never sees the foreign key.

  /**
   * Merge a diagram's frontmatter overrides over the global config.
   * @param {object} [overrides] - The config.mermaidPlugin object, if any.
   * @returns {object} The effective config for a single diagram.
   */
  function resolveDiagramConfig(overrides) {
    const merged = { ...cfg, ...(overrides || {}) };
    return {
      zoomControl: normalizeZoomControl(merged.zoomControl),
      disableMaximize: Boolean(merged.disableMaximize),
      minZoom: Number(merged.minZoom),
      maxZoom: Number(merged.maxZoom),
      containerHeight: valueToPx(merged.containerHeight),
    };
  }

  /**
   * Remove `key:` and its indented children from the frontmatter block only,
   * leaving the rest of the diagram source untouched.
   * @param {string} code - The full diagram source.
   * @param {string} key - The frontmatter key to strip.
   * @returns {string} The source with the key (and its children) removed.
   */
  function stripFrontmatterKey(code, key) {
    ...
    for (const line of m[2].split(/\r?\n/)) {
      ...
      if (childIndent !== null) {
        // Drop the key's nested children (and blank lines inside the block).
        if (line.trim() === "" || indent > childIndent) continue;
        childIndent = null;
      }
      const km = keyRe.exec(line);
      if (km) {
        childIndent = km[1].length;
        continue;
      }
      kept.push(line);
    }
    return (
      code.slice(0, m.index) + m[1] + kept.join("\n") + m[3] + code.slice(m.index + m[0].length)
    );
  }

  /**
   * Read a diagram's per-diagram config and return the effective config along
   * with the source stripped of the config.mermaidPlugin key.
   * @param {string} code - The full diagram source.
   * @returns {{cfg: object, code: string}} Effective config and clean source.
   */
  function readDiagramConfig(code) {
    let overrides;
    try {
      const fm = parseFrontmatter(code);
      overrides = fm.data && fm.data.config && fm.data.config.mermaidPlugin;
    } catch (e) {
      console.warn("[Mermaid] frontmatter parse failed; using global config", e);
    }
    return {
      cfg: resolveDiagramConfig(overrides),
      code: overrides ? stripFrontmatterKey(code, "mermaidPlugin") : code,
    };
  }

  /**
   * Collect every diagram on the page with its cleaned source and config.
   * @returns {Array<{el: HTMLElement, code: string, cfg: object}>}
   */
  function collectDiagrams() {
    return [...document.querySelectorAll(".mermaid-block .mermaid")].map((el) => {
      const resolved = readDiagramConfig(el.textContent);
      return { el, code: resolved.code, cfg: resolved.cfg };
    });
  }
  ...
  function getButtons(includeZoom, dcfg) {
    const buttons = [];

    // Zoom controls only make sense when a pan/zoom instance is driving them.
    if (includeZoom) {
      buttons.push(
    ...
    return buttons.map((btn) => btn.outerHTML).join("\n");
  }

  /**
   * Build the controls for the diagram.
   * @param {HTMLElement} block - The block element containing the diagram.
   * @param {boolean} includeZoom - Include the zoom in/out/reset buttons.
   * @param {object} dcfg - The effective per-diagram config.
   * @returns {HTMLElement} The controls element.
   */
  function buildControls(block, includeZoom, dcfg) {
    const controls = document.createElement("div");
    controls.className = "mermaid-controls";
    controls.innerHTML = getButtons(includeZoom, dcfg);
  ...
  /**
   * Set the maximized state of the diagram.
   * @param {HTMLElement} block - The block element containing the diagram.
   * @param {boolean} on - Whether to maximize the diagram.
   */
  ...
  // Escape exits full screen for whichever diagram is maximized. A block can
  // only be maximized if its own config allowed it, so no extra guard is needed.
  ...
  // ── Pan/zoom setup ───────────────────────────────────────────────────
  /**
   * Initialize svg-pan-zoom on the diagram, along with trackpad wheel handling
   * and a resize observer. This is independent of the control buttons — it
   * returns the instance so the caller can wire the zoom buttons to it.
   * @param {HTMLElement} el - The `.mermaid` element holding the rendered svg.
   * @param {SVGElement} svgEl - The rendered svg element.
   * @param {HTMLElement} block - The enclosing `.mermaid-block`.
   * @param {object} dcfg - The effective per-diagram config.
   * @returns {Promise<object|null>} The svg-pan-zoom instance, or null when
   *   pan/zoom is disabled or the svg can't be laid out.
   */
  async function initPanZoom(el, svgEl, block, dcfg) {
    if (dcfg.zoomControl === "none" || !window.svgPanZoom) return null;
    ...
    if (!hasSize(svgEl)) return null; // pan/zoom unavailable, but the static svg shows
    ...
    try {
      panZoomInstance = window.svgPanZoom(svgEl, {
        ...
        fit: true,
        center: true,
        minZoom: dcfg.minZoom,
        maxZoom: dcfg.maxZoom,
        zoomScaleSensitivity: 0.3,
        // "wheel" mode uses svg-pan-zoom's built-in wheel zoom directly. "pan"
        // mode turns it off and replaces it with our own handler below, so a
        // trackpad pinch (wheel + ctrlKey) zooms while a two-finger scroll pans.
        mouseWheelZoomEnabled: dcfg.zoomControl === "wheel",
      });
    } catch (e) {
  ...
  // ── Control wiring ───────────────────────────────────────────────────
  /**
   * Wire up a diagram's interactive controls. Zoom buttons are only added when
   * pan/zoom is active, but the maximize button is independent of pan/zoom and
   * appears whenever it isn't disabled — so turning off pan/zoom keeps maximize.
   * @param {HTMLElement} el - The `.mermaid` element holding the rendered svg.
   * @param {HTMLElement} block - The enclosing `.mermaid-block`.
   * @param {object} dcfg - The effective per-diagram config.
   */
  async function initInteractions(el, block, dcfg) {
    const svgEl = el.querySelector("svg");
    if (!svgEl) return;

    // Stash the config on the block so other handlers (Escape, resize) can read
    // this diagram's settings rather than the page-wide defaults.
    block._mermaidCfg = dcfg;

    // Per-diagram height override; falls back to the page-wide default var.
    block.style.setProperty("--mermaid-container-height", dcfg.containerHeight);
    ...
    const panZoomInstance = await initPanZoom(el, svgEl, block, dcfg);

    // Nothing to show if pan/zoom is off (no zoom buttons) and maximize is
    // disabled — skip the empty control bar.
    if (!panZoomInstance && dcfg.disableMaximize) return;

    const controls = buildControls(block, Boolean(panZoomInstance), dcfg);
  ...
  async function renderOneDiagram(el, code, dcfg, maxRetries = 5) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        await initInteractions(el, el.closest(".mermaid-block"), dcfg);
        return;
      } catch (e) {
        const elapsed = (performance.now() - t0).toFixed(1);
        console.warn(
          `%c[Mermaid] ✗ Attempt ${attempt}/${maxRetries} failed (${elapsed}ms): ${e.message}`,
  ...
  async function renderDiagrams(diagrams) {
    console.log(
      `%c[Mermaid] Found ${diagrams.length} diagram(s) to render`,
      "color: #2196F3; font-weight: bold",
    );
    const t0 = performance.now();
    for (const diagram of diagrams) {
      await renderOneDiagram(diagram.el, diagram.code, diagram.cfg);
    }
    const elapsed = (performance.now() - t0).toFixed(1);
    console.log(
  ...
  async function main() {
    ...
    // Resolve per-diagram configs up front so library loading can account for
    // per-diagram overrides (a diagram may enable pan/zoom even if the global
    // default disables it, and vice versa).
    const diagrams = collectDiagrams();

    if (diagrams.some((d) => d.cfg.zoomControl !== "none")) {
      try {
        await loadScript(
