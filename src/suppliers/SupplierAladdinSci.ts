  ...
  implements ISupplier
{
  ...
  public readonly supplierName: string = "Aladdin Sci";

  // Base URL for HTTP(s) requests
  public readonly baseURL: string = "https://www.aladdinsci.com";

  // Shipping scope for Aladdin Sci
  public readonly shipping: ShippingRange = "international";

  // The country code of the supplier.
  public readonly country: CountryCode = "US";

  protected limit: number = 60;
  ...
  protected headers: HeadersInit = {
    ...
    // "referrer": "https://www.aladdinsci.com/us_en/catalogsearch/result/?q=acetone",
    /* eslint-enable */
  };
  ...
  protected async queryProducts(
    ...
    const queryProductPage = async (query: string, page: number): Promise<Element[] | void> => {
      const searchResponse = await this.httpGetHtml({
        // https://www.aladdinsci.com/us_en/catalogsearch/result/index/?q=acetone&product_list_limit=60

        path: "us_en/catalogsearch/result/index/",
        headers: {
          referrer: `${this.baseURL}/us_en/catalogsearch/result/?q=${encodeURIComponent(query)}`,
        },
        params: {
      ...
      console.log("fuzzResults:", $fuzzResults);
      debugger;
      return $fuzzResults;
    };
  ...
  protected fuzzHtmlResponse(query: string, response: string): Element[] {
    ...
    const products = parsedHTML.querySelectorAll(".products.products-list > ol.products > li");
  ...
  protected titleSelector(data: Element): string {
    const title = data.querySelector(".product-item-details .product-item-name");
    if (title === null) {
